						...
						<table lay-filter="demo" class="layui-table">
							<thead>
								<tr>
									...
									<th lay-data="{field:'scrollImg1', align:'center'}">轮播图1</th>
									<th lay-data="{field:'scrollImg2', align:'center'}">轮播图2</th>
									<th lay-data="{field:'scrollImg3', align:'center'}">轮播图3</th>
									<th lay-data="{field:'notice'}">公告</th>
							...
							<tbody>
								...
								if (mysqli_num_rows($result) > 0) {
									while($row = mysqli_fetch_assoc($result)) {
										...
										echo "<td>{$row['id']}</td>";
								?>
								<td><img src='<?php echo $scroll_imgPrefix."{$row['scrollImg1']}"; ?>' /></td>
								<td><img src='<?php echo $scroll_imgPrefix."{$row['scrollImg2']}"; ?>' /></td>
								<td><img src='<?php echo $scroll_imgPrefix."{$row['scrollImg3']}"; ?>' /></td>
								<?php
										echo "<td>{$row['notice']}</td>";
										echo "<td>{$row['adminID']}</td>";
										echo "<td>
												<a class='layui-btn layui-btn-xs' href='ImgAndNotice-Edit.php?id={$row['id']}&notice={$row['notice']}'>修改</a>
											  </td>";
										echo "</tr>";
									}	
								}
