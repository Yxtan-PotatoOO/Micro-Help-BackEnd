...
if(!empty($keywords)){  
	$sql = "SELECT * FROM tb_LostedInfo WHERE lostedRemarks like '%{$keywords}%' ";
}else{    
	$sql = "SELECT * FROM tb_LostedInfo";
}
...
<html>
	...
	<body>
		...
		<div class="layui-layout layui-layout-admin">
				...
				<div class="layui-side-scroll">                 
					<ul class="layui-nav layui-nav-tree"  lay-filter="test">
						...
						<li class="layui-nav-item layui-nav-itemed">
							...
							<dl class="layui-nav-child">
								<dd><a href="../ThingsManagement/Losted.php" id="checked">寻物启事管理</a></dd>
			...
			<div class="layui-body contentbox">
				<div class="layui-breadcrumb breadcrumb" lay-separator=">">
					<a>失物/寻物管理</a>
					<a><cite>寻物启事管理</cite></a>
				</div>
				<div class="tablebox">
					<div class="top-box">
						<form class="querybox" action="Losted.php" method="post">
							<label>备注说明:</label>
						...
						<table lay-filter="demo" class="layui-table">
							<thead>
								<tr>
									...
									<th lay-data="{field:'imgUrl1', width:200, align:'center'}">丢失物品图片1</th>
									<th lay-data="{field:'imgUrl2', width:200, align:'center'}">丢失物品图片2</th>
									<th lay-data="{field:'lostedAddress', width:200}">丢失地点</th>
									<th lay-data="{field:'lostedTime', width:200}">丢失时间</th>
									...
									<th lay-data="{field:'comTime', width:180, align:'center', sort: true}">找回时间</th>
								</tr>
							</thead>
							<tbody>
								<?php
								if (mysqli_num_rows($result) > 0) {
									while($row = mysqli_fetch_assoc($result)) {
										...
										echo "<td>
												...
												<a class='layui-btn layui-btn-xs' href='api/Losted-action-edit.php?id={$row['id']}'>更新时间</a>
											  </td>";
								...
								if($row['imgUrl1'] != null){
								?>
								<td><img src='<?php echo $losted_imgPrefix."{$row['imgUrl1']}"; ?>' /></td>
								<?php
								}
								else{
									echo "<td></td>";
								...
								if($row['imgUrl2'] != null){
								?>
								<td><img src='<?php echo $losted_imgPrefix."{$row['imgUrl2']}"; ?>' /></td>
								<?php
								}
								else{
									echo "<td></td>";
								...
								?>
								<?php
										echo "<td>{$row['lostedAddress']}</td>";
										echo "<td>{$row['lostedTime']}</td>";
										echo "<td>{$row['lostedRemarks']}</td>";
		...
		<script type="text/javascript">
			function del (id) {
				if (confirm("确定删除这行吗？")){
					window.location = "api/Losted-action-del.php?id="+id;
				}
			}
			function reset () {
				window.location = "Losted.php";
			}
		</script>
		<script>
