...
if(!empty($keywords)){  
	$sql = "SELECT * FROM tb_tasks WHERE (status = 3 or status = 6 or status = 7) and taskRemarks like '%{$keywords}%' ";
}else{    
	$sql = "SELECT * FROM tb_tasks WHERE status = 3 or status = 6 or status = 7";
}
...
<html>
	...
	<body>
					...
					<ul class="layui-nav layui-nav-tree"  lay-filter="test">
						...
						<li class="layui-nav-item layui-nav-itemed">
							...
							<dl class="layui-nav-child">
								...
								<dd><a href="../TasksManagement/CancelTasks.php" id="checked">取消任务列表</a></dd>
						...
						<table lay-filter="demo" class="layui-table">
							<thead>
								<tr>
									...
									<th lay-data="{field:'sendCancel', width:120, align:'center'}">发单取消</th>
									<th lay-data="{field:'sendCancelReason', width:120}">发单取消原因</th>
									<th lay-data="{field:'sendCancelTime', width:180, align:'center', sort: true}">发单取消时间</th>
									<th lay-data="{field:'recCancel', width:120, align:'center'}">接单取消</th>
									<th lay-data="{field:'recCancelReason', width:120}">接单取消原因</th>
									<th lay-data="{field:'recCancelTime', width:180, align:'center', sort: true}">接单取消时间</th>
									<th lay-data="{field:'cancelTime', width:180, align:'center', sort: true}">取消时间</th>
							...
							<tbody>
								...
								if (mysqli_num_rows($result) > 0) {
									while($row = mysqli_fetch_assoc($result)) {
										...
										if($row['sendCancel'] == 1){
											echo "<td><i class='layui-icon layui-icon-close-fill correct'></i></td>";
										}else{
											echo "<td></td>";
										}
										echo "<td>{$row['sendCancelReason']}</td>";
										echo "<td>{$row['sendCancelTime']}</td>";
										if($row['recCancel'] == 1){
											echo "<td><i class='layui-icon layui-icon-close-fill correct'></i></td>";
										}else{
											echo "<td></td>";
										}
										echo "<td>{$row['recCancelReason']}</td>";
										echo "<td>{$row['recCancelTime']}</td>";
										echo "<td>{$row['cancelTime']}</td>";
										if($row['delSend']!=0){
		...
		<script type="text/javascript">
			function del (id) {
				if (confirm("确定删除这行吗？")){
					window.location = "api/CancelCancelTasks-action-del.php?id="+id;
				}
			}
			function reset () {
